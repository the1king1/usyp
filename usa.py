    ...
    async def check(self, ret=5):
        if "_" in self.email and len(self.email) <6:
            ...
            while trys < ret:
                trys += 1
                try:
                    signature = SignerPy.sign(params=self.params, cookie=self.cookies, data=self.payload)
                    self.headers.update({
...
class UserNames:
    ...
    async def search(self, session: aiohttp.ClientSession) -> list:
        ...
        self.headers.update(SignerPy.sign(params=self.params, data=self.payload))
        try:
